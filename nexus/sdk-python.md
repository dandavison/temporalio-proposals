...
A Nexus service is a collection of Nexus operations, each of which supports the four verbs `start`, `get_info`, `get_result`, and `cancel`.
...
This document focuses on these two areas (they’re the most complex aspects of Nexus SDK design). A subsequent document will cover remaining areas.
...
| --- | --- | --- | --- |
| **Service interface
shared with caller** | Does not exist | Traditional interface w/ a unary method for each op.
These signatures are used by the caller service stub, but are not implemented as such by the handler service.

`public interface NexusService {
  @Operation
  HelloOutput **hello**(HelloInput)`
 | Non-standard dataclass-style type that specifies name and input/output types without an explicit signature.

`@nexusrpc.service_interface
class MyNexusService:
  hello: NexusOperation[HelloInput, HelloOutput]`
 |
| **Service impl** | An op is an `Operation[I, O any]`.

Service is built at worker start time:

`HelloOperation = NewWorkflowRunOperation(...)
service := nexus.NewService(MyServiceName)
err = service.Register(HelloOperation)` | Every op in the interface is represented in the impl by a nullary factory method that returns `OperationHandler<T,R>` where `T` and `R` match the signature in the interface (registration-time check).

`@ServiceImpl(service = NexusService.class)
public class NexusServiceImpl {
  
  @OperationImpl
  **OperationHandler**<HelloInput,
                   HelloOutput> **hello**()` | Similar to Java: every op in the interface is represented by a nullary factory method that returns an instance of a class implementing the `Operation[I, O]` interface, with import-time check that these match the types in the interface.

`@nexusrpc.handler.service(MyInterface)
class MyService:
  
  @nexusrpc.handler.operation
  def hello(self) -> nexusrpc.handler.Operation[HelloInput,
                                                HelloOutput]:
      return HelloOperation(self)`
 |
| **Convenience shorthand
for an operation backed by a sync function that only implements start** | `NewSyncOperation` constructor takes start method as an anonymous function:

`EchoOperation = nexus.**NewSyncOperation**(
  name,
  func(ctx, EchoInput, StartOperationOptions)
      (EchoOutput, error) {
    ...
    return EchoOutput(input), nil
})`
 | Implement factory method as usual, but use `sync` operation constructor with an anonymous function to define the start method.

`@OperationImpl
OperationHandler<EchoInput,
                 EchoOutput> echo() {
 ...
 | Implement start method directly on the service class;
decorator converts it to factory method returning operation.

`@nexusrpc.handler.**sync_operation**
async def echo(self, input: EchoInput,
               _: StartOperationOptions) -> EchoOutput:
  ...
  return EchoOutput()`
 
Python lambdas do not allow a multi-statement block. If we wanted this to be more like Java, then that might look like this:

**`@nexusrpc.handler.operation
def echo(self) -> nexusrpc.handler.Operation[EchoInput,
                                             EchoOutput]:
  async def start(self, input: EchoInput,
                 _: StartOperationOptions) -> EchoOutput:
    ...
    return EchoOutput()
  ...
  return SyncOperation(start)`
 |
| **Convenience shorthand for a workflow-backed operation that implements start and cancel** | `NewWorkflowRunOperation` constructor takes workflow and an anonymous function that returns the workflow options.

`HelloOperation = **NewWorkflowRunOperation**(
  name,
  HelloWorkflow,
  func(ctx, HelloInput,StartOperationOptions)
      (StartWorkflowOptions, error) {
    return client.StartWorkflowOptions{
      ID: options.RequestID,
    }, nil
})` | Implement factory method as usual, but use `fromWorkflowMethod` operation constructor, passing an anonymous function that returns a workflow stub run method.

`@OperationImpl
OperationHandler<HelloInput,
                 HelloOutput> hello() {
  return **WorkflowRunOperation.fromWorkflowMethod**(
    (ctx, details, input) -> myStub::hello
  )
}`
 | Implement start method directly on the service class;
decorator converts it to factory method returning `Operation`.
...
`@temporalio.nexus.handler.**workflow_operation**
 async def hello(self, input: HelloInput,
                 options: StartOperationOptions)
      -> WorkflowStartOperationResult[HelloOutput]:
  return await temporalio.nexus.handler.start_workflow(
      HelloWorkflow.run, input, "workflow-id", options
)` |
| **Full Operation interface**
 | Each verb is a method with parameters:
1. Go ctx
2. The input for that verb (op input or token)
3. A verb-specific options struct

Note that start return type supports sync and async return types.

`type **Operation**[I, O any] interface {
  
  **Start**(ctx, I,
        StartOperationOptions)
      (HandlerStartOperationResult[O], error)
  ...
  **Cancel**(ctx, token,
      ...
      error
}` | Each verb is a method whose signature contains (1) OperationContext (shared Nexus headers), and (2) a verb-specific details struct. Input is pulled out as a separate parameter for start.
...
`public interface **OperationHandler**<T, R> {
  ...
  OperationInfo
    **fetchInfo**(OperationContext,
              OperationFetchInfoDetails)
  
  R
    **fetchResult**(OperationContext,
 ...
 | Most similar to Go: each verb is a method with parameters:
1. The input for that verb (op input or token)
2. A verb-specific options struct

`class **Operation**(Protocol, Generic[I, O]):
    
  async def **start**(self, I,
                  StartOperationOptions)
        -> Union[O, StartOperationResult]
    
 
  async def **fetch_info**(self, token,
                       FetchOperationInfoOptions)
        -> OperationInfo
    
  async def **fetch_result**(self, token,
                         FetchOperationResultOptions)
        -> O
    
   async def **cancel**(self, token,
                    CancelOperationOptions) -> None: ...`
 |